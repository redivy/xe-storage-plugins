...
# written to the logs.
def pread(cmdlist, close_stdin = False, scramble=None):
    cmdlist_for_exec = []
    cmdlist_for_log = []
    for item in cmdlist:
        if is_string(item):
...
def sr_get_driver_info(driver_info):
    ...
    results['capabilities'] = driver_info['capabilities']
...
def get_attached_VBDs(session, vdi_uuid):
    """get all VBDs for the given VDI that are currently plugged. The caller
    must ensure that no VBDs are currently in a transitioning state (with
    current-operations=plug)"""
    vbds = dict()
    vdi_ref = session.xenapi.VDI.get_by_uuid(vdi_uuid)
    vbd_records = session.xenapi.VBD.get_all_records_where( \
            "field \"VDI\" = \"%s\"" % vdi_ref)
    for vbd_ref, vbd_rec in vbd_records.iteritems():
        if vbd_rec["currently_attached"]:
            vbds[vbd_ref] = vbd_rec
    return vbds

def get_hosts(session, vbds):
    host_list = []
    for vbd_ref, vbd_rec in vbds.iteritems():
        if not vbd_rec.get("VM"):
            continue
        vm = session.xenapi.VM.get_record(vbd_rec["VM"])
        host_ref = vm["resident_on"]
        if host_ref != "OpaqueRef:NULL" and host_ref not in host_list:
            host_list.append(host_ref)
    return host_list
