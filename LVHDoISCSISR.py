...
class LVHDoISCSISR(LVHDSR.LVHDSR):
    ...
    def handles(type):
        if sys.argv[0].endswith("LVMoISCSI"):
            return type == "lvmoiscsi" # for the initial switch from LVM
        if type == "lvhdoiscsi":
            return True
        return False
        ...
        if self.dconf['target'].find(',') == 0 or self.dconf['targetIQN'] == "*":
            ...
            try:
                ...
                if pbd <> None and not self.dconf.has_key('multiSession'):
                    self.dconf['multiSession'] = IQNstring
                    self.session.xenapi.PBD.set_device_config(pbd, self.dconf)
            except:
                pass
